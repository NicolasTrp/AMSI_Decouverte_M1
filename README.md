...
# 🦄 Lab CTF Découverte M1 — « Licornia Parc »
...
Version **découverte** (Master 1) du lab avancé « Licornia Parc ». L'étudiant part du
**seul point d'entrée** fourni (IP + port du site web) et doit **compromettre 3 machines**
en chaîne, en faisant le **tour des outils de base** et en touchant **toutes les catégories
classiques de CTF** (Web, OSINT, Crypto, Forensique, Stégano, Réseau/Pivot, Reverse, Pwn,
Privesc).
...
|---|---|---|---|
| `web01` (`web`)        | DMZ `…10.0/24` | Site vitrine + portail — **point d'entrée** | ✅ `WEB_PUBLISH_IP:PORT` |
| `firewall`            | DMZ + LAN      | Routeur/pare-feu (segmentation + pivot)     | ❌ interne |
| `fileshare` (`files`) | LAN `…20.0/24` | Partage SMB (RH, sauvegardes)               | ❌ interne (via pivot) |
...
|---|---|
| web01 | Recon/Énum · **Web** · **OSINT** · **Encodage** · **Privesc** |
| fileshare | **Réseau/Pivot** · **Forensique** · **Stégano** · **Crypto/Cracking** · **Privesc** |
| admin | **Pwn** (exploitation binaire) · **Reverse** · trophée final |

10 flags `AMSI{...}`. Spine obligatoire (linéaire) + flags bonus (OSINT, forensique,
stégano). Détail réservé encadrant : [`docs/walkthrough.md`](docs/walkthrough.md).
...
├── services/
│   ├── web/                    # Apache/PHP : injection cmd, OSINT/EXIF, base64, sudo GTFOBins
